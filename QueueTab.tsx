  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={load}
          className="text-xs font-mono-data text-muted border border-panelLine rounded-full px-3 py-1.5 hover:text-paper hover:border-signal-accent transition"
        >
          refresh
        </button>
      </div>
        ...
        {records.map((r) => (
          <div key={r.id} className="border border-panelLine rounded-xl p-5 bg-panel/60 space-y-3">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-muted mt-0.5">
                  {r.diagnosis} → {r.requested_treatment}
                </p>
              </div>
            ...
            <p className="text-xs font-mono-data text-muted">
              confidence {r.weighted_confidence_score}
            </p>
            ...
            {r.reviewer_decision ? (
              ...
              <div className="flex items-center gap-3 pt-2">
                <input
                  ...
                  className="flex-1 bg-ink/60 border border-panelLine rounded-lg px-3 py-1.5 text-sm text-paper focus:border-signal-accent transition"
                />
                <button
                  onClick={() => decide(r.id, "ACCEPT")}
                  className="px-4 py-1.5 rounded-full bg-signal-pass/15 text-signal-pass border border-signal-pass/40 text-sm hover:bg-signal-pass/25 transition"
                >
                  Accept
                </button>
                <button
                  onClick={() => decide(r.id, "OVERRIDE")}
                  className="px-4 py-1.5 rounded-full bg-signal-fail/15 text-signal-fail border border-signal-fail/40 text-sm hover:bg-signal-fail/25 transition"
                >
                  Override
                </button>
              </div>
