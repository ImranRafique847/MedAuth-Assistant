...
export default function PipelineTrace({
  ...
  return (
    <div className="w-full overflow-x-auto">
      ...
      >
        <path d={pathD} fill="none" stroke="#22304A" strokeWidth={1.5} />
        <path
          d={pathD}
          fill="none"
          stroke="#5B8DEF"
        ...
        {STAGES.map((stage, i) => {
          ...
          return (
            <g key={stage.key}>
              ...
              <circle cx={x} cy={midY} r={9} fill="none" stroke={color} strokeWidth={1} opacity={0.4} />
              <text x={x} y={midY + 32} textAnchor="middle" fontSize="11"
                fontFamily="IBM Plex Mono, monospace" fill="#8C97AD">
                {stage.label}
              </text>
              <text x={x} y={midY - 20} textAnchor="middle" fontSize="9"
                fontFamily="IBM Plex Mono, monospace" fill={color} fontWeight={500}>
                {status || "—"}
              </text>
            </g>
          );
